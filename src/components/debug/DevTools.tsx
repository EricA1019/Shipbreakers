...
export default function DevTools() {
  ...
  const [activeTab, setActiveTab] = useState<'logs' | 'state' | 'diagnostics'>('logs');
  ...
  const downloadBugReport = () => {
    const issues = diagResults.length ? diagResults : Diagnostics.validateGameState(state);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-8">
      <div className="bg-zinc-900 border-2 border-amber-600 w-full h-full max-w-5xl max-h-[80vh] flex flex-col shadow-2xl rounded-lg overflow-hidden">
        ...
        <div className="bg-zinc-950 p-3 border-b border-amber-600/30 flex justify-between items-center">
          ...
          <button onClick={() => setIsOpen(false)} className="text-zinc-500 hover:text-amber-500">✕</button>
        </div>
        ...
        <div className="flex-1 overflow-hidden bg-zinc-900/90 font-mono text-xs">
          {activeTab === 'logs' && (
            <div className="h-full flex flex-col">
              <div className="flex-1 overflow-y-auto p-4 space-y-1">
                {logs.map((log) => (
                  <div key={log.id} className="flex gap-2 hover:bg-zinc-800/50 p-0.5 rounded">
                    <span className="text-zinc-500 shrink-0 w-20">{new Date(log.timestamp).toLocaleTimeString()}</span>
                    <span className={`shrink-0 w-12 font-bold ${
                      log.level === 'ERROR' ? 'text-red-500' :
                      log.level === 'WARN' ? 'text-amber-400' :
                      log.level === 'INFO' ? 'text-blue-400' : 'text-zinc-400'
                    }`}>{log.level}</span>
                    <span className="text-zinc-300 break-all">
                      {log.message}
                      {log.meta && <span className="text-zinc-500 ml-2">{JSON.stringify(log.meta)}</span>}
                      {log.stack && <pre className="text-red-400/70 mt-1 ml-4 text-[10px]">{log.stack}</pre>}
                    </span>
                  </div>
                ))}
                <div ref={logsEndRef} />
              ...
              <div className="p-2 border-t border-amber-600/20 flex justify-between bg-zinc-950">
                <button onClick={() => logger.clear()} className="text-zinc-400 hover:text-white px-2">Clear</button>
                <button onClick={() => logger.download()} className="text-amber-500 hover:text-amber-400 px-2">Download Export</button>
              </div>
            </div>
          )}
            ...
            <div className="h-full overflow-y-auto p-4">
              <pre className="text-green-400/80 whitespace-pre-wrap">
                {JSON.stringify(state, (key, value) => {
                  if (key === 'grid') return `[Grid ${value.length}x${value[0]?.length}]`; // Summarize grid
                  return value;
                }, 2)}
              </pre>
            </div>
          ...
          {activeTab === 'diagnostics' && (
            <div className="h-full p-4">
              ...
              <div className="text-zinc-300 mb-3">{diagTimestamp ? `Last run: ${diagTimestamp}` : 'Not run yet'}</div>
