...
export const Diagnostics = {
  validateGameState: (state: GameState) => {
    ...
    if (state.currentRun.timeRemaining < 0) {
      issues.push(
        `timeRemaining is negative: ${state.currentRun.timeRemaining}`,
      );
    }

    const wreck = state.availableWrecks.find(
      (w) => w.id === state.currentRun?.wreckId,
    );
    if (!wreck) {
      ...
      if (!ship) {
        ...
        if (!ship.grid || !Array.isArray(ship.grid)) {
          ...
          if (
            ...
            if (!entryRoom) {
              issues.push(
                `Entry position (${entry.x},${entry.y}) points to null room`,
              );
            }
          }
        }
    ...
    state.crewRoster.forEach((crew) => {
      ...
      if (crew.hp < 0 || crew.hp > crew.maxHp) {
        issues.push(
          `Crew ${crew.name} has invalid hp ${crew.hp}/${crew.maxHp}`,
        );
      }
    });
